export const siteConfig = {
    siteName: import.meta.env.PUBLIC_SITE_NAME,
    siteUrl: import.meta.env.PUBLIC_SITE_URL,
}

interface NavItem {
    label: string;
    href: string;
    target?: string; // target es opcional, por defecto es _self o no se especifica
}

interface Nav {
    avatar?: string;
    items?: NavItem[]; // Usa un arreglo para almacenar los elementos de navegación y facilitar su adición o eliminación
}

// Definir los elementos de navegación
export const nav: Nav = {
    avatar: '/assets/sho.jpeg',
    items: [
        { label: 'Inicio', href: '/', target: '_self' },
        { label: 'Presentación', href: '/presentation', target: '_self' },
        { label: 'Evidencias', href: '/project', target: '_self' },
    ],
};

// Pie de página
export const footerText = `© ${new Date().getFullYear()} Sahian Castro, todos los derechos reservados.`

// SEO - Título, descripción y palabras clave
interface SeoTdk {
    title?: string
    description?: string
    keywords?: string
}

// SEO de la página de inicio
export const homeTdk: SeoTdk = {
    title: 'Sahian Castro | Portafolio',
    description: 'Estudiante de ingeniería de sistemas',
    keywords: 'Sayis, Sahian, portafolío'
}

// SEO del blog
export const blogTdk: SeoTdk = {
    title: 'Aprendizajes | Sahian Castro',
    description: 'Notas y aprendizajes de mi proceso académico',
    keywords: 'Sahian Castro, aprendizajes'
}

// SEO de la página "Sobre mí"
export const aboutTdk: SeoTdk = {
    title: 'Sobre mí | Sahian Castro',
    description: 'Conoce más sobre Sahian Castro, sus intereses, habilidades y proceso de aprendizaje',
    keywords: 'Sahian Castro, sobre mí, estudiante'
}

// SEO de la página de proyectos
export const projectTdk: SeoTdk = {
    title: 'Evidencias | Bases de Datos 1',
    description: 'Evidencias y trabajos realizados durante la asignatura Bases de Datos 1',
    keywords: 'Bases de Datos 1, modelado, normalización, SQL, proyecto'
}

// SEO de la página 404
export const notFoundTdk: SeoTdk = {
    title: '404 No encontrado - Aquí no hay nada.',
    description: '404 No encontrado - Aquí no hay nada.',
    keywords: '404 No encontrado. Aquí no hay nada.'
}

// Redes sociales: name = nombre, url = enlace, icon = icono SVG
export const socialLinks = [
    {
        name: 'GitHub',
        url: 'https://github.com/sahianrcastro',
        icon: `<svg t="1730125604816" class="icon ic-github ic-social" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="12741" width="256" height="256"><path d="M511.957333 21.333333C241.024 21.333333 21.333333 240.981333 21.333333 512c0 216.832 140.544 400.725333 335.573334 465.664 24.490667 4.394667 32.256-10.069333 32.256-23.082667 0-11.690667 0.256-44.245333 0-85.205333-136.448 29.610667-164.736-64.64-164.736-64.64-22.314667-56.704-54.4-71.765333-54.4-71.765333-44.586667-30.464 3.285333-29.824 3.285333-29.824 49.194667 3.413333 75.178667 50.517333 75.178667 50.517333 43.776 75.008 114.816 53.333333 142.762666 40.789333 4.522667-31.658667 17.152-53.376 31.189334-65.536-108.970667-12.458667-223.488-54.485333-223.488-242.602666 0-53.546667 19.114667-97.322667 50.517333-131.669334-5.034667-12.330667-21.930667-62.293333 4.778667-129.834666 0 0 41.258667-13.184 134.912 50.346666a469.802667 469.802667 0 0 1 122.88-16.554666c41.642667 0.213333 83.626667 5.632 122.88 16.554666 93.653333-63.488 134.784-50.346667 134.784-50.346666 26.752 67.541333 9.898667 117.504 4.864 129.834666 31.402667 34.346667 50.474667 78.122667 50.474666 131.669334 0 188.586667-114.730667 230.016-224.042666 242.090666 17.578667 15.232 33.578667 44.672 33.578666 90.453334v135.850666c0 13.141333 7.936 27.605333 32.853334 22.869334C862.250667 912.597333 1002.666667 728.746667 1002.666667 512 1002.666667 240.981333 783.018667 21.333333 511.957333 21.333333z" p-id="12742"></path></svg>`
    },
];


// Etiqueta de página: etiqueta que aparece en la parte superior de cada página
interface PageTag {
    index: string
    about: string
    blog: string
    project: string
}

export const pageTag: PageTag = {
    index: 'PORTAFOLIO',
    about: 'SOBRE MÍ',
    blog: 'APRENDIZAJES',
    project: 'EVIDENCIAS'
}


// Descripción de página: texto que aparece debajo del título principal de cada página
interface PageDescription {
    index?: string
    project?: string
    blog?: string
    about?: string
}

export const pageDescription: PageDescription = {
    index: 'Soy Sahian, estudiante de ingeniería de sistemas interesada en la tecnología, el desarrollo y la innovación. Este espacio reúne mi proceso de aprendizaje y las evidencias de mi trabajo durante Bases de Datos 1',
    project: 'En esta sección econtrarás las evidencias de mi proceso de aprendizaje en Bases de Datos 1: modelado, normalización, SQL y proyecto',
    about: 'Soy estudiante y constantemente actualizo mis conocimientos en toda el área de ingeniería de sistemas',
    blog: 'Un espacio para compartir mis aprendizajes, experiencias y reflexiones durante mi formación académica',
}


// Elementos de filtro: lista de filtros para mostrar los proyectos en la página de inicio
export interface FilterItem {
    content: string
    dataGroup: string
}

export const filterItems: FilterItem[] = [
	{ content: "Modelado", dataGroup: "modelado" },
	{ content: "Normalización", dataGroup: "normalizacion" },
	{ content: "SQL", dataGroup: "sql" },
	{ content: "Proyecto", dataGroup: "proyecto" },
];